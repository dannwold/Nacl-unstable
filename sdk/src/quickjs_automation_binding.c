...
static JSValue js_auto_context_ctor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    JSAutoContext *ac = calloc(1, sizeof(JSAutoContext));
...
static int js_automation_init(JSContext *ctx, JSModuleDef *m) {
    ...
    JS_SetPropertyFunctionList(ctx, proto, js_auto_context_proto_funcs, sizeof(js_auto_context_proto_funcs)/sizeof(js_auto_context_proto_funcs[0]));
