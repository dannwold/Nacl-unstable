...
int telephony_parse_registry_dumpsys(const char *dump, CellTowerMetric *out_cells, int max_count, int *out_count);
...
static JSValue js_telephony_get_cells(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    telephony_parse_registry_dumpsys(
        "CellIdentityLte:{mMcc=310 mMnc=260 mCi=2390812 mPci=312 mTac=14232 mEarfcn=66661}",
        cells,
        8,
        &count
    );
...
static int js_telephony_init(JSContext *ctx, JSModuleDef *m) {
    ...
    JS_SetPropertyFunctionList(ctx, proto, js_telephony_proto_funcs,
        sizeof(js_telephony_proto_funcs) / sizeof(js_telephony_proto_funcs[0]));
    JS_SetClassProto(ctx, js_telephony_class_id, proto);

    return 0;
}

JSModuleDef *js_init_module_telephony(JSContext *ctx, const char *module_name) {
    JSModuleDef *m = JS_NewCModule(ctx, module_name, js_telephony_init);
    if (!m) return NULL;
    JS_AddModuleExportList(ctx, m, js_telephony_proto_funcs, sizeof(js_telephony_proto_funcs) / sizeof(js_telephony_proto_funcs[0]));
    return m;
}
